...
A secure legal document and digital evidence management system for police
case files: encrypted storage, version history that never overwrites,
mandatory redaction on protected cases, a tamper-evident audit trail, and
a hash anchored to a ledger for every file.

This file is the handover document. `RUNNING.md` covers running the demo,
`DEPLOY.md` covers hosting, and **`openapi.yaml` is the contract for every
endpoint** - change it in the same commit as the route.

---

## 1. Run it locally

You need Node 20+ and PostgreSQL 14+ (tested on 18).
